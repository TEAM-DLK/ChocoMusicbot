...
from CHOCO.DLK import get_string
from CHOCO.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
                    SOUNCLOUD_IMG_URL, STREAM_IMG_URL,
                    TELEGRAM_AUDIO_URL, TELEGRAM_VIDEO_URL,
                    MUSIC_BOT_NAME, adminlist)
from CHOCO import YouTube, app
from CHOCO.core.call import CHOCOh
from CHOCO.misc import SUDOERS, db
from CHOCO.utils import bot_sys_stats
from CHOCO.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from CHOCO.utils.formatters import seconds_to_min
from CHOCO.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from CHOCO.utils.stream.autoclear import auto_clean
from CHOCO.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await CHOCOh.pause_stream(chat_id)
        ...
        await CHOCOh.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await CHOCOh.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await CHOCOh.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await CHOCOh.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
